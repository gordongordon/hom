...
export default class TestListView extends React.Component {
  ...
  render() {
    ...
    const row = (rowData, sectionID, rowID) => {
      ...
      return (
        <div key={rowID} className="row" onClick={()=> console.log( 'onClick')}>
          <div className="row-title">{obj.title}</div>
          <div style={{ display: '-webkit-box', display: 'flex', padding: '0.3rem 0.3rem' }}>
            <img style={{ height: '3.28rem', marginRight: '0.3rem', marginLeft: '0.3rem' }} src={obj.img} alt="icon" />
            <div className="row-text">
              <div style={{ marginBottom: '0.16rem', fontWeight: 'bold' }}>{obj.des}</div>
              <div><span style={{ fontSize: '0.6rem', color: '#FF6E27' }}>35</span>¥</div>
              <div><span style={{ fontSize: '0.6rem', color: '#FF6E27' }}>88</span>$<Button type="primary" size="sm">Hello</Button></div>
            </div>
          </div>
        </div>
      );
