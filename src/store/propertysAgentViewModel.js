...
//                that.propertys.delete( snapshot.key );
  ...
  getMatchProperty = (id, compareTo, valueTo ) => {
    ...
    // Handle match propertys
     Fb.app.agentsRef.orderByChild(compareTo).equalTo(valueTo).on("child_added", function(snap) {
...
//          Fb.app.propertysRef.update( { snap.key : { } })
     ...
     Fb.app.agentsRef.orderByChild(compareTo).equalTo(valueTo).on("child_removed", function(snap) {
...
const propertysAgent = new PropertysAgentViewModel();
export {propertysAgent};
